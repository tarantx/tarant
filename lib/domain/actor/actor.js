 ...
 */
class Actor {
    static get(id, context){
        let result = context.system.getActor(id)
        if(!(result instanceof this))
            throw `Actor with id ${id} does not exist in this system`
        return result
    }

    static create(id, initialState = {}, context = {}) {
        if (context.system.getActor(id))
            throw `Actor with id ${id} already exists in this system`
        _lock = Math.random()
        return new this(id, initialState, context, _lock)
    }

    constructor(id, initialState = {}, context = {}, lock) {
        if(_lock != lock) 
        ...
        this.materializer = ActorSystem.composeMaterializers(ActorSystem.EmptyMaterializer, context.materializer);
     ...
     */
    tell(actor, message) {
        (actor.receiveMessage ? actor : this.system.getActor(actor)).receiveMessage({ origin: this, message: message });
    }
     ...
     */
    ask(actor, message) {
        if (actor.id === this.id || actor === this.id) {
        ...
        return new Promise((resolve, reject) => {
            ...
            (actor.receiveMessage ? actor : this.system.getActor(actor)).receiveMessage(newMessage);
        });
    }
