...
class Mailbox {
    ...
    pull(transaction) {
        ...
        return new Promise((_resolve, _reject) => {
            ...
            let reject = (error) => _reject(this.__triggerMessageFailed(messageToProcess, error, this.queue));
    ...
    peek() {
        if (this.queue.length === 0) {
            ...
            return this.supportQueue[0];
        }
        return this.queue[0];
    }

    discard() {
        this.queue = this.queue.slice(1).concat(this.supportQueue);
        this.supportQueue = [];
    }
